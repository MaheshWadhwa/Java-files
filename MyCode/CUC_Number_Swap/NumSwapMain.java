...
import CUC_Number_Swap.DataClean_Main;


import com.google.common.collect.Table.Cell;
public class NumSwapMain 

{
...

	{
		...
		outputpath="C:\\Automated_Execution\\Output\\CUC_NumberSwap\\".concat(output);
		...
		for(i=1;i<iterations;i++)
		{
			...
			File Activation=new File (data,"NumberSwap");
			Activation.mkdir();
			...
			try
			{
				...
				else
				{
					...
					int cleantcheck=dclean.dataCleanmain();
					Thread.sleep(5000);
					if(cleantcheck>0)
					{
				...
				CUC_NumberSwap cuc=new CUC_NumberSwap();
				int numswapcheck=cuc.numberswap();
				if (numswapcheck==0)
				{
					report.writeToFile(reportfile,"Number swap done successfully");
					report.writeTorun(runlog,"Number swap done successfully");
				...
				{
					report.writeToFile(reportfile,"Number swap failed and order is cancelled");
				}
