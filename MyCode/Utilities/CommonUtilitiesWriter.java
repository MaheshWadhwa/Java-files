...
//		writer.println("Clicked on "+strOpt);
	...
	public void selectOptionDD(WebDriver driver,String selectXpath,String opt){
		WebElement select1 = new WebDriverWait(driver,30).until(ExpectedConditions.elementToBeClickable(By.xpath(selectXpath)));
		List<WebElement> options1 = select1.findElements(By.tagName("option"));
		for (WebElement option : options1) {
			if(option.getText().contains(opt)) {
				option.click();
